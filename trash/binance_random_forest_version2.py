...
class TradingBot:
    ...
    async def start_code(self):
        ...
        pbar = tqdm(total=len(self.param_space), desc="Bayesian Search")
        while True:
            try:
                ...
                if self.model is None:
                    self.model = self.train_model(features, labels, self.param_space, pbar)
                else:
    ...
    def prepare_labels(df):
        ...
        future_prices = np.roll(prices, -1)  
        labels = np.where(future_prices[:-1] > prices[:-1], 1, -1)
        labels = np.append(labels,0)
        return labels

    def train_model(self, features, labels, param_space, pbar):
        model = BayesSearchCV(RandomForestClassifier(), param_space, n_iter=50, cv=3, n_jobs=-1)   
    ...
    def train_model_with_existing_model(self, features, labels):
        if self.model.best_score_ < self.model.score(features, labels):
